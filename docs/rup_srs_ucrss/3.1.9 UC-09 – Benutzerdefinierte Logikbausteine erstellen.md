...
1. **Benutzer → Editor:** Der Benutzer wählt im Menü die Option **„Neuer Baustein“**.  
2. **Editor → Benutzer:** Das System öffnet den Editor, der Eingabefelder für Pins, Symbol und Logik bereitstellt.  
3. **Benutzer → Editor:** Der Benutzer legt die Eigenschaften und Logik des Bausteins fest (z. B. Wahrheitstabelle oder logische Ausdrücke).  
4. **Editor → Compiler:** Die definierte Logik wird an den Compiler übergeben.  
5. **Compiler → Editor:** Der Compiler überprüft die Logik.  
...
6. **Compiler → Bibliothek:** Bei Erfolg wird der neue Baustein gespeichert.  
7. **Bibliothek → Benutzer:** Der neue Baustein erscheint in der Bausteinliste und steht zur weiteren Verwendung bereit.
